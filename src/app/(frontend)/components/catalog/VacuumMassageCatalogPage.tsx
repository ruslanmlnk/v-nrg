...
const catalogItems: CatalogItem[] = featuredCatalogProducts.map((product, index) => {
  const isBaseModel = product.id === 'v-nrg-18-pro'

  return {
    ...product,
    category: 'vacuum',
    listFeatures: isBaseModel
      ? ['18 маніпул', 'Потужність 800 Вт', 'Для помірного навантаження']
      : ['36 маніпул', 'Потужність 1200 Вт', 'Для інтенсивної роботи'],
    maniples: isBaseModel ? 18 : 36,
    powerWatts: isBaseModel ? 800 : 1200,
    summary: isBaseModel
      ? 'Професійний апарат для кабінету та одного спеціаліста'
      : 'Посилена конфігурація для салону з вищим навантаженням',
...
export default function VacuumMassageCatalogPage() {
  ...
  const [selectedCategory, setSelectedCategory] = useState<CategoryFilter>('vacuum')
  const [selectedModels, setSelectedModels] = useState<ProductId[]>([
    'v-nrg-18-pro',
    'v-nrg-36-pro',
  ])
  const [selectedManiples, setSelectedManiples] = useState<Array<18 | 36>>([18, 36])
  ...
  const filteredProducts = catalogItems.filter((item) => {
    ...
    const matchesModel = selectedModels.includes(item.id)
    const matchesManiples = selectedManiples.includes(item.maniples)
  ...
  const resetFilters = () => {
    ...
    setSelectedModels(['v-nrg-18-pro', 'v-nrg-36-pro'])
    setSelectedManiples([18, 36])
        ...
        <section className="grid items-start gap-5 lg:grid-cols-[400px_minmax(0,1fr)]">
          <aside className="flex flex-col gap-5">
            ...
            <FilterSection title="Модель">
              {(['v-nrg-18-pro', 'v-nrg-36-pro'] as ProductId[]).map((modelId) => {
                const count = catalogItems.filter((item) => item.id === modelId).length
                ...
                return (
                  <CheckboxOption
                    ...
                    count={count}
                    label={modelId === 'v-nrg-18-pro' ? 'V-NRG 18 PRO' : 'V-NRG 36 PRO'}
                    onClick={() =>
                      setSelectedModels((current) => toggleSelection(current, modelId))
                    }
                  />
                )
              })}
            ...
            <FilterSection title="Кількість маніпул">
              {([18, 36] as Array<18 | 36>).map((maniples) => {
