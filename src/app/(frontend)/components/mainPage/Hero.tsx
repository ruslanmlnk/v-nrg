  ...
  return (
    <section className="mx-auto mt-[23px] grid max-w-[1288px] grid-cols-1 gap-5 px-6 md:mt-5 md:grid-cols-2">
      <div className="flex flex-col rounded-[20px] bg-[#22354A] px-6 py-8 md:py-12 md:pl-8 md:pr-7">
        ...
        <ArrowPillButton className="mr-[50px] mt-6 shrink-0 self-center whitespace-nowrap px-5 md:mr-0 md:mt-[164.5px] md:self-start md:px-6">
          Отримати консультацію
        </ArrowPillButton>
      </div>
      <div className="relative min-h-[360px] overflow-hidden rounded-[20px]">
        <Image src={image} alt={title} fill className="object-cover" sizes="(min-width: 768px) 50vw, 100vw" priority />
      </div>
    </section>
  )
}
